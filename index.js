...
app.post('/api/messages', (req,res) => {
  ...
  if (!username || !message || !time) return res.status(400).json({ error: 'Missing required fields' });
  const newMsg = { id: uuidv4(), username, message, time, seed: seed||'', reactions: reactions||{} };
...
app.post('/api/pass', (req,res) => {
  ...
  if (password === "min113") { chatMessages = []; io.emit('clearMessages'); return res.status(200).json({ message: "全てのメッセージを削除しました。" }); }
...
io.on('connection', (socket) => {
  ...
  socket.on('sendMessage', (data) => {
    const newMsg = { id: uuidv4(), username: data.username||'匿名', message: data.message||'', time: data.time||nowISO(), seed: data.seed||'', reactions: {} };
    chatMessages.push(newMsg);
    io.emit('newMessage', newMsg);
  });
  ...
  socket.on('joinGame', ({ roomId, username }) => {
    ...
    if (!rooms[roomId].game) {
      rooms[roomId].game = { board: createInitialBoard(), turn: 1, status: 'waiting', players: {}, lastMove: null, counts: countPieces(createInitialBoard()) };
    }
  ...
  socket.on('makeMove', ({ roomId, r, c, color }) => {
    ...
    if (hasAnyLegalMove(game.board, opponent)) game.turn = opponent;
      ...
      game.winner = result.winner; // 1 black, 2 white, 0 draw
      game.counts = result.counts;
    }
    // always include legalMoves for current turn (if playing)
    io.to(roomId).emit('gameState', enrichGameState(game));
  });
